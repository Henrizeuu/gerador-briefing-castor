...
def rodar_extracao(url_insta, url_maps):
    ...
    # ---------------------------------------------------------
    # 1. EXTRAÇÃO DO INSTAGRAM (Híbrida com Fotos do Feed)
    # ---------------------------------------------------------
    try:
        ...
        run_input_barato = {
            ...
            "postsPerProfile": 12, # Baixa 12 posts para garantir material pro portfólio
            "proxy": {
                "useApifyProxy": True,
        ...
        for item in client.dataset(run_barato.default_dataset_id).iterate_items():
            # Captura dados principais do usuário (vem embutido nos posts)
            if "user" in item and not nome_completo:
                nome_completo = item["user"].get("full_name", "")
                foto_perfil_url = item["user"].get("profile_pic_url", "")
            
            # Recria a estrutura de pastas por shortcode para as imagens e legendas
            if "code" in item:
                shortcode = item["code"]
                ...
                # Extrai a URL da imagem (seja post único ou carrossel)
                img_url = ""
        ...
        # Salva o arquivo de texto principal do perfil
    ...
    # ---------------------------------------------------------
    # 2. EXTRAÇÃO DO GOOGLE MAPS (Playwright Original e Intacto)
    # ---------------------------------------------------------
    if url_maps:
        try:
